...
STOCK_LIST = ['MRF', 'PAGEIND', 'BOSCHLTD', '3MINDIA', 'HONAUT', 'ABBOTINDIA', 'SHREECEM', 'JSWHL', 'POWERINDIA', 'PTCIL',
...
class LadderEngine:
    ...
    def select_top_movers(self):
        ...
        idle_stocks = [
            s for s in self.active_stocks.values() 
            if s.status == "IDLE" and s.ltp > 0 and s.turnover >= min_turnover
        ]
        
        if not idle_stocks:
            return
            
        # Sort by % Change (gainers)
        idle_stocks.sort(key=lambda x: x.change_pct, reverse=True)
        top_gainers = idle_stocks[:self.settings.top_n_gainers]
        
        # Sort by % Change (losers)
        idle_stocks.sort(key=lambda x: x.change_pct)
        top_losers = idle_stocks[:self.settings.top_n_losers]
